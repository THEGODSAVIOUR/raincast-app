...
def predict():
    ...
    if not readings: return jsonify({"error": "No readings provided"}), 400
    results = []
    for r in readings:
        try:
            air_temp, dew_point = float(r.get("air")), float(r.get("dew"))
        except (TypeError, ValueError):
            return jsonify({"error": "Invalid input format"}), 400
        rh = calculate_relative_humidity(air_temp, dew_point)
        results.append({"air_temp": air_temp, "dew_point": dew_point, "relative_humidity": round(rh, 2)})
    
    sorted_results = sorted(results, key=lambda x: x["relative_humidity"])
    highest = max(results, key=lambda x: x["relative_humidity"])
    
    first_air, last_air = float(readings[0]["air"]), float(readings[-1]["air"])
    ...
    return jsonify({"sorted_results": sorted_results, "highest": highest, "prediction": prediction})
